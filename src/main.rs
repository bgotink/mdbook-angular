...
fn main() -> Result<(), Error> {
	...
	renderer.render(RenderContext::from_json(io::stdin())?)?;
...
impl AngularRenderer {
	fn prepare(&self, ctx: &mut RenderContext) -> Result<AngularWorker, Error> {
	...
	fn render(&self, mut ctx: RenderContext) -> Result<(), Error> {
		let actual_renderer = HtmlHandlebars::new();
		let worker = self.prepare(&mut ctx)?;
...
impl AngularWorker {
	fn new(ctx: &RenderContext) -> Result<AngularWorker, Error> {
	...
	fn process_chapter(&mut self, chapter: &mut Chapter) -> Result<(), Error> {
		let mut angular_code_samples: Vec<String> = Vec::new();
			...
			match &e {
				...
				pulldown_cmark::Event::End(pulldown_cmark::Tag::CodeBlock(
					...
					if let Some(angular_code) = &current_angular_code_block {
						angular_code_samples.push(angular_code.clone());

						// TODO get tag name
						let tag_name = format!("codeblock-{}", angular_code_samples.len());

						return vec![
							e,
							pulldown_cmark::Event::Html(pulldown_cmark::CowStr::from(format!(
								"<{0}></{0}>\n",
								&tag_name
							))),
						];
